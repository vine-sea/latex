...
\usetikzlibrary{shapes,backgrounds,arrows,automata,decorations.pathreplacing,calligraphy,calc}
...
MST性质:$N=(V,\{E\}),U \subseteq V,u\in U,v \in V-U,(u,v)$是具有最小权值的边，则存在最小生成树包含$(u,v)$

\tp{
    \node[scale=.6,black,draw,shape=circle,name=a] at (0,0) {$u^{'}$};
    \node[scale=.6,black,draw,shape=circle,name=b] at (0,1) {$u$};
    \node[scale=.6,black,draw,shape=circle,name=c] at (1,0) {$c$};
    \node[scale=.6,black,draw,shape=circle,name=d] at (1,1) {$v$};
    \node[scale=.6,black,draw,shape=circle,name=e] at (.5,.5) {$v^{'}$};

    \draw (a)--(b);
    \draw (a)--(e);
    \draw (e)--(d);
    \draw (c)--(d);
    \draw [loosely dashed] (b)--(d);

    \draw [ color=red ,dashed](.25,-.2) --(.25,1.2);
}
...
\q 选择 $u\in U,v_1\in V-U, min\{ (u,v_1) \}$，

\q 更新$U=\{ u_0 ,v_1\},TE=\{(u,v_1) \}$,

\q 重复选择至U=V

\q $closedge[i-1].lowcost=Min\{cost(u,v_i) | u \in U \}$
...
\q 初始状态 $T=(V,\{  \})$,每个顶点自成连通分量

\q 选择 $e_i=MIN(E),e_i$ 若属于不同连通分量更新，否则舍弃

\q 更新$E=e-e_i,T=(V,\{ e_i \})$

\q 舍弃$E=e-e_i$ 

\q 重复选择至T中所有顶点在一个连通分量上
...
\tp{
    \coordinate (o) at (0,0);
    \coordinate (l) at (270-20:1cm);
    \coordinate (r) at (270+20:1cm);

   
    \coordinate (fl) at ($(o)+(l)$) ;
    \coordinate (fr) at ($(o)+(r)$) ;
    \coordinate (sl) at ($(o)+(l)+(l)$) ;
    \coordinate (sr) at ($(o)+(l)+(r)$) ;


    \node[black,draw,shape=circle,name=no] at (o) {$$};
    \node[black,draw,shape=circle,name=nfl] at (fl) {$$};
    \node[black,draw,shape=circle,name=nfr] at (fr) {$$};
    \node[black,draw,shape=circle,name=nsl] at (sl) {$$};
    \node[black,draw,shape=circle,name=nsr] at (sr) {$$};
